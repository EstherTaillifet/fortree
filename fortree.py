...
class Fortree:
    ...
    def init_tree_var(self):
        ...
        self.modules_list = [ md.Module(one_module[0], one_module[1]) for one_module in pre_mod_list[1:]]


        # Build root
        if self.program:
            self.root = tn.TreeNode(self.program, self.directory, self.routines_list, self.modules_list, self.fortree_type, keyword="program", root_path=self.root_path)
        elif(self.module):
            self.root =  tn.TreeNode(self.module, self.directory, self.routines_list, self.modules_list, self.fortree_type, keyword="module", root_path=self.root_path)
        elif(self.routine):
            self.root =  tn.TreeNode(self.routine, self.directory, self.routines_list, self.modules_list, self.fortree_type, keyword="routine", root_path=self.root_path)
        else:
        ...
        if(self.fortree_type == "CALL_TREE"):
            ...
            self.tree_arr = np.array(["parent", "defined"], dtype='U200')
        else:
        ...
        if(self.fortree_type == "CALL_TREE"):
            ...
            if (np.size(self.root.children) > 1):
                for child in self.root.children[1:]:
                    self.tree_arr=np.vstack([self.tree_arr, [self.root.name, child[0]]])
    ...
    def build_node(self, parent_node_obj, level=False):
        ...
        if (np.size(parent_node_obj.children) > 2):
            for child in parent_node_obj.children[1:]:
                ...
                node =  tn.TreeNode(child[0], self.directory, self.routines_list, self.modules_list, self.fortree_type, keyword="routine", root_path=child[1])
